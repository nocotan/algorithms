...
bool dfs(int x, int v, int vmax) {
    bool res = false;
    if(x+1>=field.size()-1&&vmax>=field[x+1]-field[x]) return true;
    if(v==0 && vmax < field[x+1]-field[x]) return false;
    v = vmax;

    res = dfs(field[x+1], v-(field[x+1]-field[x]), v) || dfs(field[x+1], 0, v/2);
    return res;
}
