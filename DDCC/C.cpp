...
int dfs(string s, int cost, int f) {
    string tmp = "";
    rep(i,s.length()) {
        if (s[i]=='0') tmp += '1';
        else tmp += '0';
    ...
    if (s.length()>=t.length()) {
        if (s==t) return cost;
        else if(f==1) return INF;
        else if (t==tmp) return cost+c;
        else return INF;
    }
    int res_a = dfs("0"+s, cost+a, 0);
    int res_b = dfs(s+"1", cost+b, 0);

    int res = INF;
    if (f == 0) {
        int res_c = dfs(tmp, cost+c, 1);

        if (res_a < res_b) res = res_a;
        else res = res_b;
        if (res>res_c) res = res_c;
    }
    else {
        res = min(res_a, res_b);
    }
