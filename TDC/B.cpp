...
int dfs(int i, int j) {
    ...
    if(i==A&&j==B) res = 0;
        ...
        if(i==A) res = dfs(i, j+1) + b[j];
        else if(j==B) res = dfs(i+1, j) + a[i];
        else res = max(dfs(i+1, j)+a[i], dfs(i, j+1)+b[j]);
    }
    else {
        if(i==A) res = dfs(i, j+1);
        else if(j==B) res = dfs(i+1, j);
        else res = min(dfs(i+1, j), dfs(i, j+1));
    }
